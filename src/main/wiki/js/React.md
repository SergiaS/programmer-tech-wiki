...
У завантажених проектах буде відсутня тека `node_modules` яка зберігає всі залежності від проекту і важить достобіса.
Тому її не включають у commit'и, і доводиться завантажувати відсутні залежності на свій ПК самому.

Спочатку переходимо у теку завантаженого проекту і потім пишемо у терміналі команду `npm install`.
Чекаємо поки завантажаться всі залежності з файлу проекту `package.json`, і все.
...
Далі через **props** визивають потрібний атрибут.
...
Техніко все ок, але іноді це не буте спрацьовувати.
І це не дуже хороша практика оновлення стану.

Кожного разу коли ви оновлюєте стан і ви по якійсь причині залежите від минулого стану,
...
Просто потрібно додати атрибут в input який слухаємо і оновлювати стан при відправці даних з форми:
...
1. Додавання стилю динамічно. Вказати **inline**. Такий спосіб вказування `.css`-стилю має найвищій пріорітет - будуть перевизначені усі стилі які були до цього!
    ...
    Коли задаються стилі **CSS** в синтасисі `.jsx` треба писати їх у подвійних фігурних дужках `{{}}`.
    Значення пишуть в лапках завжди, коли це не змінна.
    Cтилі пишуть в лапках `''` тільки якщо є дифіс `-`, тобто так: `{{'background-color': 'yellow'}}`,
...
Конструкція записується у зворотніх лапках `` {`...`} `` всередині фігурних дужок.
...
* [Приклади дивися у проекті на GitHub](https://github.com/SergiaS/c_react_styling_components)
...
2. Використовувати CSS-модуль - для цього проек повинен бути налаштований спеціальним чином:
