...
> JS is garbage collected. JS автоматично прибере з пам'яті об'єкти на яких немає посилань, але не Map та Set якщо там будуть
> посилкові об'єкти. Для автоматичного прибирання сміття з мап і сету використовуй `WeakMap` та `WeakSet`.
>
> Коли викликаєш функцію з примітивним значенням - це значення зберігається у CallStack - це коротко термінована пам'ять браузера.
