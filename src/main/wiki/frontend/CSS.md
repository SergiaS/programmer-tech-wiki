...
> Щоб задати свої стилі для тегу `<hr>`:
> ```css
> hr {
>   border: none;
>   height: 1px;
...
### overflow
Hide any child element if they overlap this container:
```css
.smbd {
  overflow: hidden;
}
...
* `align-content` - призначений для вирівнювання багато строкових по CROSS вісі.
  ...
  Тут повинен бути контейнер висотою більше, а ніж сума його елементів - використовуй `width` або `min-height: 100vh`.
...
* `align-self` - задає вирівнювання конкретного елемента по CROSS вісі в середині контейнера.
  ...
  * `stretch` - розтягує елемент по всій висоті контейнера.
...
> Автоматичну компіляцію в Intellij IDEA можна зробити за допомогою плагіну **File Watchers**.
> Докладна інструкція з налаштуваннями є 
> [тут](https://www.jetbrains.com/help/idea/transpiling-sass-less-and-scss-to-css.html#ws_sass_less_scss_example_scss)
>
> коротко - щоб отримати `.css` з `.scss`:
> 1. Встановити **Node.js** та встановити наступні пакети:
>    - `npm install -g sass`
>    - `npm install --global less`
> 2. Для налаштування компіляції у теку `/css` - треба задати наступні значення:
>    - у поле **Arguments** введи: `$FileName$:$ProjectFileDir$/css/$FileNameWithoutExtension$.css` - замість `$FileName$`
>      можна підставити ім'я конкретного файлу, наприклад `main.scss` - компіляція буде тільки цього файлу;
>    - у поле **Output paths to refresh** введи: `$ProjectFileDir$/css/$FileNameWithoutExtension$.css:$ProjectFileDir$/css/$FileNameWithoutExtension$.css.map`
...
* [Прклад створення ієрархії навігації - breadcrumb](https://codyhouse.co/demo/breadcrumbs-multi-steps-indicator/index.html)
