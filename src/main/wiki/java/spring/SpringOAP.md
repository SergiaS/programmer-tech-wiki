# Spring AOP
> У Spring транзакції підтримуються на рівні аспектів.

> Для використання **Spring AOP** необхідно додати нові залежності (`spring-aop` і `aspectjtools`) у файл `pom.xml`.

> Незважаючи на те, що використовується Spring AOP, все одно потрібно підключити `AspectJ` - Spring AOP використовує деякі AspectJ класи та анотації.


AOP (Аспектно-ориентированное программирование, Aspect Oriented Programming) -
...
Прописав Joint Point в параметры метода Advice, мы получаем доступ к информации о сигнатуре и параметрах метода с бизнес-логикой.
```java
