...
По умолчанию у методов это `this`.
...
Когда мы работаем с synchronized методами, если метод не статичный, то всегда идёт синхронизация на объекте this. То же самое что и synchronized блок, где явно указывается используемый монитор конкретного объекта.*__
